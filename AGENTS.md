...
.env                 optional Relay pairing token and Jev OpenRouter key; LOCAL, gitignored; presence-gates section 14 and the shadow-only Jev wake triage (docs/configuration.md "Jev shadow wake triage")
...
config/knowledge-store  optional knowledge-store name (line 1), search-instructions clause (line 2), and optional backend key (line 3) for generated crewmate briefs and /retrospective; LOCAL, gitignored; absent keeps bin/fm-brief.sh's historical upstream RAG wording; inherited by secondmate homes; see docs/configuration.md "Knowledge store naming (config/knowledge-store)"
...
state/               runtime records and signals; gitignored
  ...
  <id>.advisor-count  per-worktree reviewer-call counter of the opt-in advisor Stop hook (bin/fm-advisor-hook.sh); written only when the task's dispatch profile carried advisor; removed by teardown
  <id>.grok-turnend-token   firstmate-owned grok hook registry token for the task; removed by teardown
  <id>.kimi-turnend-token   firstmate-owned Kimi hook registry token for the task; removed by teardown
  <id>.muse-session  muse busy-source binding (sessions root plus task worktree) written by fm-spawn; removed by teardown
  <id>.cursor-session  cursor busy-source binding (projects root, task worktree, prior conversations) written by fm-spawn; removed by teardown
  <id>.agy-session   agy busy-source binding (conversations root, task worktree, prior conversations) written by fm-spawn; removed by teardown
  <id>.agy-session-current  cached agy conversation resolution derived from <id>.agy-session; written by the busy fold; removed by teardown
...
`docs/configuration.md` owns dispatch-profile and runtime-backend schemas, including a profile's optional opt-in per-turn advisor axis, `bin/fm-harness.sh` owns static resolution, and `bin/fm-spawn.sh` owns launch flags and fail-closed validation.
...
A crewmate never writes to a shared knowledge store itself: a ship or scout brief has it raise each durable finding as a `note: CANDIDATE - {finding}` status line, so routing that candidate to its owner above is firstmate's work, on the single drain that presents it (section 3).
...
A decision is simply a task held for the captain: `tasks-axi hold <id> --reason "<reason>" --kind captain`, with `--until <date>` when the captain defers it; the reason is the decision card `captain-hold-lifecycle` owns.
...
- `captain-hold-lifecycle` - load before holding a task for the captain, before treating an investigation or visual review as complete, before ending a visual review that exposed a captain decision, when recording or routing the captain's answer, and on any `RECORD DIVERGENCE` line from the wake drain.
...
- `goplanner-skills` - load before starting work on the GOPlanner project to check its existing project-specific skills first.
- `aws-goplanner-skills` - load before starting work on the aws-goplanner project to check its existing project-specific skills first.
- `goplanner-android-skills` - load before starting work on the goplanner-android project to check its existing project-specific skills first.
...
Load `writing-for-agents` before updating this file.
