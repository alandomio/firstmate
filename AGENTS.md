...
For PR-based ship tasks, the ready signal depends on mode: `no-mistakes` reports `done: PR <url> checks green` after CI is green, while `direct-PR` reports `done: PR <url>` after opening the PR. A brief generated for a GitLab project carries the same signal in merge-request wording - `done: MR <url> checks green` and `done: MR <url>` - so treat the `PR`/`MR` noun as the project's forge vocabulary, not a different contract.
