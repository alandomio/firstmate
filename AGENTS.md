...
config/knowledge-store  optional two-line knowledge-store name (line 1) and search-instructions clause (line 2) for generated crewmate briefs; LOCAL, gitignored; absent keeps bin/fm-brief.sh's historical upstream RAG wording; inherited by secondmate homes; see docs/configuration.md "Knowledge store naming (config/knowledge-store)"
