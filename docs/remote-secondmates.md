...
The primary resolves the verified secondmate harness and optional model and effort, runs the same readiness gate the seed runs, transfers the inherited-material allowlist, best-effort rsyncs the engineer's skills (see [Remote secondmate skill sync](configuration.md#remote-secondmate-skill-sync-configskill-sync-exclude)), and asks the remote host to launch on Herdr in `fm-remote`.
