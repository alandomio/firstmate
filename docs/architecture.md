...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers whose `FM_PAUSE_RESURFACE_SECS` recheck finds their situation changed, their agent exited, or `FM_PAUSE_REMIND_SECS` passed since their last surface, and heartbeat backstop hits.
...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake; live or inconclusive liveness remains fail-open at that initial surface, so a still-live agent's declared wait is shown once rather than silenced.
Once that first surface has recorded the pause, backend liveness no longer gates its classification: the declared `paused:` or durable `captain-held` endpoint stays on the same bounded cadence even while the pane content churns, and only `bin/fm-crew-state.sh` reporting the crew working lifts it, so a dead or a live agent alike takes the decorated recheck instead of repeated bare stale wakes (`tests/fm-watch-triage.test.sh`).
That first surface also records the pause's situation - its status log identity, the agent's liveness, and the pane text with digits and blanks removed - and the same line never takes a second one-time surface after a busy turn.
Each later due recheck compares against that record: an unchanged situation is absorbed into the watcher's triage log until `FM_PAUSE_REMIND_SECS` has passed since the last surface, while new pane text such as a dialog or a new status line surfaces at that recheck with the change named.
An exited agent is never absorbed: it surfaces at every recheck, naming the exit the first time.
...
That library's `status_is_nonterminal_progress_verb` is the single owner of which verbs stay nonterminal, informational `note:` among them, and the captain-relevance test and both daemon wedge guards read it rather than keeping enumerations that can drift apart.
...
A merge request with no head pipeline at all is still refused unless a live read of the project's own `jobs_enabled`/`builds_access_level` confirms CI cannot run there for any commit; [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns that exception's exact conditions.
...
Ship briefs forbid crewmates from writing those files directly; a crewmate raises durable project-intrinsic findings as a `note: CANDIDATE - {finding}` status line for firstmate to route, and only firstmate writes them, on the captain's explicit confirmation. `data/projects.md` stays a thin private registry.
