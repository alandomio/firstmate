...
Each later due recheck compares against that record: an unchanged situation is absorbed into the watcher's triage log until `FM_PAUSE_REMIND_SECS` has passed since the last surface, while an exited agent, new pane text such as a dialog, or a new status line surfaces at that recheck with the change named.
