...
| `fm-knowledge-store.sh`  | Print the home's resolved org-wide knowledge store (`name`, `backend`, `search`) for skills such as `/retrospective` |
| `fm-knowledge-store-lib.sh` | Single owner of reading `config/knowledge-store` and deriving its backend, shared by the brief and the read command |
...
| `fm-advisor-hook.sh`     | Opt-in crewmate/scout Stop-hook advisor: second-model per-turn review installed by fm-spawn only when a dispatch profile carries `advisor` (docs/configuration.md "Crew dispatch profiles") |
...
| `fm-jq-lib.sh`           | Single owner of passing fleet-sized JSON values to jq over stdin instead of argv |
...
| `fm-jev.sh`              | Opt-in, shadow-only Jev wake classifier: status, the measurement report, masking, and the detached classification of presented wakes |
| `fm-jev-lib.sh`          | Hook-side Jev helpers: the `.env` switch and the silent event appends the drain, sends, lifecycle actions, captain holds, and turn ends make |
