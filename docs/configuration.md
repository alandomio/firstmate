...
agy (Antigravity CLI) is verified for crewmate and scout launches ONLY under the same rule and for the same reason - it exposes no hook, plugin lifecycle, or app-server surface a primary session's turn-end supervision could be built on - so `{"harness": "agy"}` is a valid dispatch profile value and `fm-spawn.sh` refuses it for a secondmate; [runtime backend verification](verification/runtime-backends.md) owns its evidence.
agy accepts `low`, `medium`, and `high` effort only, and its busy-state fold reads agy's own conversation database through the `sqlite3` CLI, which is not guaranteed present on a fleet machine: without it the fold degrades to `unknown` rather than erroring, so confirm `command -v sqlite3` wherever agy is dispatched.
An agy worker also does not discover the captain's user-level Claude skills directory at all - it reads only its own workspace-relative `.agents/skills`, a global path, or a shared path under its own config home - so a skill an agy crewmate is expected to invoke must be mirrored into one of those paths first.
...
`glab` is required on top of that list for any home whose projects live on GitLab: it covers merge-request operations the way gh-axi covers GitHub, firstmate's own merge watch and merge path shell out to it, and a brief dispatched against a GitLab project tells its crewmate to use it.
Bootstrap does not check for `glab`, so a home missing it finds out only when a GitLab merge request watch refuses with `error: watching a GitLab merge request requires glab on PATH` (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)); install it with `brew install glab` or the platform's package manager.
...
FM_PROC_ROOT_OVERRIDE=   # alternate /proc root for Linux process-identity reads in fm-wake-lib.sh and fm-teardown.sh and for fm-teardown.sh's leftover-process fallback scan, mainly for tests
...
FM_TEARDOWN_PROCESS_SCAN_TIMEOUT_SECS=30   # whole seconds allowed per leftover-process scan inside fm-teardown.sh; a failed or capped lsof scan switches to reading /proc on Linux, and teardown refuses, --force included, only when that cannot answer either; the cap bounds scans that can be killed, and an lsof stuck in uninterruptible kernel sleep can still hold teardown past it
FM_BEARINGS_QUOTA_TIMEOUT=20   # seconds bounding the one quota-axi read inside fm-bearings-quota.sh; a timeout degrades the board's quota panel to unavailable
...
FM_CLASSIFY_RESOLVE_VERB=resolved  # leading status verb that closes a keyed needs-decision/blocked item; excluded from FM_CAPTAIN_RE
FM_CLASSIFY_CAPTAIN_HELD_VERB=captain-held   # leading status verb for a verified durable-backlog transfer; excluded from FM_CAPTAIN_RE
# These three overrides are additive, not replacements: the nonterminal-progress-verb set matches the built-in literal (paused, resolved, captain-held) AND the configured verb, because firstmate's own writers keep emitting the literal
...
FM_PAUSE_REMIND_SECS=86400         # once a declared wait has surfaced, the watcher absorbs each due recheck whose status log, agent liveness, and pane text (ignoring digits and blanks) are unchanged since that surface, and surfaces it again as a reminder only after this many seconds; any change surfaces at the next recheck, and an exited agent keeps surfacing on every recheck; the away-mode daemon does not read it
