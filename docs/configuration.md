...
FM_PAUSE_REMIND_SECS=86400         # once a declared wait has surfaced, the watcher absorbs each due recheck whose status log, agent liveness, and pane text (ignoring digits and blanks) are unchanged since that surface, and surfaces it again as a reminder only after this many seconds; any change surfaces at the next recheck; the away-mode daemon does not read it
