...
FM_TEARDOWN_PROCESS_SCAN_TIMEOUT_SECS=30   # whole seconds allowed per leftover-process scan inside fm-teardown.sh; a failed or capped lsof scan switches to reading /proc on Linux, and teardown refuses, --force included, only when that cannot answer either
