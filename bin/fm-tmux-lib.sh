...
# separated (pi/agy) composer shape, tmux's analogue of herdr's native
# `agent get`. It answers only for pi or agy, from two live signals:
...
#     runtime-backends.md "Agent liveness name sources") or the agy process
#     (verified live, agy 1.1.28: tmux reports its own comm directly as
#     "agy", no node-wrapper ambiguity to resolve, unlike cursor-agent),
...
#   - status: the matched agent's verified busy footer via fm_pane_is_busy,
