...
# bounds that home's startup-memory curation, primary config/knowledge-store
# makes a secondmate's own generated crewmate briefs name the same knowledge
# store instead of regressing to the upstream default, and primary
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context knowledge-store}"
