...
detect_own() {
  ...
  # agy (Antigravity CLI) is checked BEFORE claude for the identical reason
  # cursor is: agy does NOT clear an inherited CLAUDECODE (or AI_AGENT), so an
  # agy subprocess launched from a claude primary carries BOTH markers and
  # whichever is tested first wins. Verified live, agy 1.1.28: a tool
  # subprocess launched under a claude primary carried ANTIGRAVITY_AGENT=1,
  # ANTIGRAVITY_AGENTAPI_EXE=<path to agy>, CLAUDECODE, and AI_AGENT
  # simultaneously. bin/fm-spawn.sh additionally clears the foreign markers at
  # the launch boundary; both are kept for the same reason as cursor's split
  # above (launch sanitization only covers sessions fm-spawn started).
  ...
  for _ in 1 2 3 4 5 6 7 8; do
    ...
    case "$(basename -- "$comm")" in
      ...
      *grok*) echo grok; return ;;
      ...
      # agy (Antigravity CLI) resolves to an unwrapped binary and tmux reports
      # its comm literally as "agy" (verified live, agy 1.1.28) - no
      # node-wrapper obfuscation to resolve, unlike cursor-agent, so an exact
      # match is enough.
      agy) echo agy; return ;;
