...
family_for_basename() {
  case "$1" in
    ...
    fm-bearings-board.test.sh|fm-bearings-quota.test.sh|\
    ...
    fm-wake-drain-unread-status.test.sh|fm-jev.test.sh|\
    ...
    fm-send-secondmate-marker.test.sh|fm-shared-captain-inheritance.test.sh)
      printf '%s\n' secondmate
      ;;
    fm-bootstrap.test.sh|fm-fleet-sync.test.sh|fm-gate-refuse.test.sh|fm-gotmp.test.sh|fm-handoff.test.sh|\
    ...
    fm-pal-council-catalog-live-e2e.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-backend.sh|bin/fm-backend-hometag-lib.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-watch*|bin/fm-wake*|bin/fm-inactive-reconcile.sh|bin/fm-jev*|\
    ...
    docs/configuration.md|docs/supervision-protocols/*)
      printf '%s\n' pure-contract-unit
      ;;
    tests/lib.sh|tests/*-helpers.sh|tests/*-harness.js)
