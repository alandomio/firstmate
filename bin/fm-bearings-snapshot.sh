...
Default fields: schema, home, generated, prs, in_flight{id,kind,state,doing},
  ...
  (a decision summary keeps up to 800 characters of its hold reason, room for a
  full decision card, and marks a longer one with a trailing ellipsis),
...
# shellcheck disable=SC2016 # jq program text, not shell expansion
MODEL=$(fm_jq -n \
  --argjson canonical_snapshot "$SNAP" \
  --arg home "$HOME_LABEL" \
  --arg now "$NOW" \
  ...
  --argjson candidate_prs "$CANDIDATE_PRS" '$canonical_snapshot |
  ...
  | ([ .backlog.records[]
         ...
         | {id,key:.id,verb:"captain-hold",
            summary:((.title + ": " + .hold_reason) | trunc(800)),owner:"(main)"} ]
     + [ (.secondmate_current.records // [])[] as $m | $m.decisions_open[]?
         | select(.source == "backlog" and .verb == "captain-hold")
         | select(($all_decisions == 1) or (.deferred_marker != true))
         | {id:($m.id + "/" + .id),key,verb,
            summary:(((.summary // .id) + ": " + (.reason // "captain decision pending")) | trunc(800)),owner:$m.id} ]) as $decisions_all
  | ([ .backlog.records[]
