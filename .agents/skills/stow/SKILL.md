...
- A project's existing committed `AGENTS.md`, for project-intrinsic knowledge useful to nearly every session of that project, raised by a crewmate as a `note: CANDIDATE - {finding}` status line for firstmate to route and write on the captain's explicit confirmation.
...
3. **Write within the existing boundaries.**
   ...
   - Project-intrinsic knowledge never goes directly into a project's `AGENTS.md`.
     A crewmate raises it as a `note: CANDIDATE - {finding}` status line for firstmate to route and write on the captain's explicit confirmation.
