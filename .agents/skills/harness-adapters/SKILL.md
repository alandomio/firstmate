...
agy AUTO-UPDATED itself from 1.1.28 to 1.2.0 partway through this single verification session, without being asked.
Everything dated 1.1.28 above was measured before that; the background-task composer strip immediately above, and the two findings fixed later in the same run (the `(step_type, status)` busy pair and the `-wal` binding scan), were all observed on 1.2.0.
Whoever verifies or re-verifies this adapter next should read every version number here as the version at the moment of that observation, and should re-check `agy --version` at the START of the pass and again at the end - agy's TUI can change under a running verification session through its own update mechanism, which is exactly how the task strip appeared mid-session here.
