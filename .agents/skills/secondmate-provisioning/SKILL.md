...
A remote launch also rsyncs the engineer's `~/.claude/skills/` into the route at this same convergence point, a separate best-effort step outside this allowlist; see [Remote secondmate skill sync](../../../docs/configuration.md#remote-secondmate-skill-sync-configskill-sync-exclude) for its contract.
